...
- `/api/dodo/webhook` verifica la firma y publica el proyecto tras confirmar un pago de la sesión y el producto correspondientes. Si Dodo cobra en otra moneda, el ranking conserva la oferta original en USD. Los eventos verificados se registran y se reconcilian periódicamente. Un reembolso retira el proyecto. Los checkouts pendientes no bloquean importes: cuando hay ofertas iguales, gana el puesto superior el pago confirmado primero.
- `/cuenta/` ofrece acceso por enlace de correo de un solo uso, sin contraseña. El correo debe coincidir con el que Dodo confirmó en el pago. Desde allí el dueño puede corregir la URL, actualizar el logo y hacer una oferta mayor para la misma ficha. La nueva oferta cobra el importe completo indicado; la ficha solo sube cuando el webhook confirma ese pago. Un reembolso de la oferta adicional restaura el mayor importe aún pagado.
- El envío de enlaces admite Postmark (preferido cuando Resend está suspendido) y Resend. Para Postmark, verificar `eneltop.com`, obtener un **Server API Token** del flujo transaccional y guardarlo en `/var/lib/eneltop/postmark-server-token` (lectura para `www-data`, modo 0600). El adaptador usa `https://api.postmarkapp.com/email` y tiene prioridad sobre Resend. Para Resend, la clave va en `/var/lib/eneltop/resend-api-key`. Opcionalmente configurar `ENELTOP_MAIL_FROM`; por defecto usa `EnElTop <acceso@eneltop.com>`. Sin clave, la ruta de solicitud responde 503 y no envía ningún enlace. Los tokens caducan a los 15 minutos, son de un solo uso y las sesiones duran 30 días.
