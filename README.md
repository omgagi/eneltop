...
- El envío de enlaces usa Resend. Para activarlo, verificar el dominio `eneltop.com` en Resend y guardar su clave API privada en `/var/lib/eneltop/resend-api-key` (lectura para `www-data`, modo 0600). Opcionalmente configurar `ENELTOP_MAIL_FROM`; por defecto usa `EnElTop <acceso@eneltop.com>`. Sin clave, la ruta de solicitud responde 503 y no envía ningún enlace. Los tokens caducan a los 15 minutos, son de un solo uso y las sesiones duran 30 días.
