...
async function loadInbox(openThreadId=''){
  ...
  for(const thread of data.threads){
    ...
    if(thread.contactLogo){const image=document.createElement('img');image.src=thread.contactLogo;image.alt='';image.onerror=()=>{image.remove();avatar.textContent=(thread.contactName||'?').charAt(0).toUpperCase()};avatar.textContent='';avatar.append(image)}
    ...
    const connect=document.createElement('button');connect.type='button';connect.className='connect-action';
    if(thread.connectionState==='connected'){connect.textContent='✓ Conectados';connect.classList.add('pending');connect.disabled=true}
    else if(thread.connectionState==='pending_sent'){connect.textContent='Solicitud enviada';connect.classList.add('pending');connect.disabled=true}
...
function contactCard(item,request=false){
  const card=document.createElement('article');card.className='contact-card';
  let avatar;if(item.logo){avatar=document.createElement('img');avatar.src=item.logo;avatar.alt=''}else{avatar=document.createElement('span');avatar.className='contact-placeholder';avatar.textContent=(item.name||'?').charAt(0).toUpperCase()}
  const info=document.createElement('div');info.className='contact-card-info';const name=document.createElement('strong');name.textContent=item.name;const state=document.createElement('span');state.textContent=request?'Quiere conectar contigo':item.online?'Conectado ahora':'Contacto';info.append(name,state);
  const actions=document.createElement('div');actions.className='contact-actions';
  if(request){const accept=document.createElement('button');accept.textContent='Aceptar';accept.addEventListener('click',()=>connectionAction(item.threadId,'accept'));const decline=document.createElement('button');decline.className='secondary';decline.textContent='Rechazar';decline.addEventListener('click',()=>connectionAction(item.threadId,'decline'));actions.append(accept,decline)}
  else if(item.threadId){const open=document.createElement('a');open.className='button';open.href='#inbox';open.textContent='Mensaje';open.addEventListener('click',()=>loadInbox(item.threadId));actions.append(open)}
  card.append(avatar,info,actions);return card;
}
async function loadConnections(){
  const data=await api('/api/account/connections'),requests=$('connection-requests'),contacts=$('contacts');requests.replaceChildren();contacts.replaceChildren();
  for(const item of data.requests)requests.append(contactCard(item,true));for(const item of data.contacts)contacts.append(contactCard(item));
  $('contacts-empty').hidden=Boolean(data.requests.length||data.contacts.length);
}
async function refreshInbox(){
  ...
  for(const box of openBoxes){
    ...
    presence.textContent=presenceText(thread);
